...
| 20 | 15 May 2026 | Antigravity (AI) & User | **Multi-Tenant Investor Platform Implementation.** Implemented Supabase Auth (backend/frontend), `profiles` table with unique handles, "Bio Link" routing (`/:handle`), and per-investor CRM dashboard. Refactored submission flow for investor-specific routing and enforced RLS database policies for strict data isolation. Built new Landing, Login, Signup, and Branded Submission pages. |
